...
### Публикация в Homebrew (macOS / Linux)

Пользователям macOS и Linux удобно ставить утилиту через Homebrew. Для этого нужно создать собственный tap (репозиторий GitHub) и написать формулу.

1. Создайте репозиторий с именем `homebrew-book-translator`.
2. Добавьте файл формулы `book-translator.rb`. Укажите там ссылку на архив с кодом, SHA256 хэш и инструкции по установке.
3. В блоке `install` формулы пропишите `virtualenv_install_with_resources`. Это автоматически подтянет Python-зависимости.

После настройки пользователи смогут устанавливать утилиту так:

```bash
brew tap your-username/book-translator
brew install book-translator
```

### GitHub Releases

Готовыми сборками удобно делиться через GitHub Releases.

1. Соберите проект в один исполняемый файл с помощью `PyInstaller` или `cx_Freeze`. Пользователям больше не придется ставить Python.
2. Создайте новый релиз в репозитории GitHub.
3. Прикрепите собранные бинарные файлы для Windows, macOS и Linux к релизу.

### Совместимость с Windows

В нативной среде Windows часто возникают проблемы с кодировкой. Python там по умолчанию использует `cp1252`. Это ломает чтение японских иероглифов.

Мы настоятельно рекомендуем запускать проект через WSL2 (Windows Subsystem for Linux). WSL2 дает полноценное Linux-окружение с правильной поддержкой UTF-8.

Если вам нужно запустить скрипт прямо в Windows, всегда явно указывайте `encoding='utf-8'` при работе с файлами. Дополнительно настройте кодировку терминала перед запуском:
