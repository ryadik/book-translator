...
#### Перевод файла или директории (`translate`)
Запускает процесс перевода для конкретного файла или директории. Скрипт автоматически найдет конфигурацию серии в родительских директориях.
...
* `--force`: Принудительно пересоздает чанки, удаляя старое состояние.
* `--resume`: Возобновляет прерванный перевод (поведение по умолчанию).
* `--dry-run`: Показывает план перевода без реальных вызовов API.
* `--stage`: Позволяет принудительно перезапустить процесс с определенного этапа (`discovery`, `translation`, `proofreading`, `global_proofreading`).
* `--docx` / `--no-docx`: Принудительно включить или отключить конвертацию итогового файла в формат `.docx`.

#### Перевод всех томов (`translate-all`)
Позволяет перевести сразу все тома в рамках одной серии.

```bash
book-translator translate-all
```
Также поддерживает флаги `--debug`, `--force`, `--resume`, `--dry-run`, `--docx`, `--no-docx`.
...
Отображает детализированный статус текущего процесса перевода для всей серии или отдельного тома, выводя красивые таблицы в терминале (через библиотеку `rich`).
